        ...
        if($blog) {
            if(!empty($title)) {
                if($blog->role === 'Admin' OR $blog->userID === $post->authorID) {
                    $postID = $userObj->update('posts', ['title' => $title, 'description' => $description, 'slug' => $slug, 'content' => $content, 'postStatus' => 'published', 'postType' => 'Page', 'isComments' => 'blocked'], ['postID' => $post->postID, 'blodID' => $blog->blogID]);
                }
            }
        }
    }
