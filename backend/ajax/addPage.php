...
if($_SERVER['REQUEST_METHOD'] === "POST") {
    if(isset($_POST['blogID'])) {
        ...
        if($blog) {
            if(!empty($title)) {
                $postID = $userObj->create('posts', ['title' => $title, 'description' => $description, 'slug' => $slug, 'authorID' => $blog->userID, 'blogID' => $blog->blogID, 'content' => $content, 'postStatus' => 'published', 'postType' => 'Page', 'isComments' => 'blocked', 'createdDate' => $date]);
            }
        }
    }
}