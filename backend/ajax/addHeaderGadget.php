...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    if (isset($_POST['blogID'])) {
        ...
        $gadget = $userObj->get('gadgets', ['blogID' => $blog->blogID, 'type' => 'header', 'displayOn' => $area, 'position' => $pos]);

        if ($blog) {
            if ($blog->role === "Admin") {
                $content = '{"title": "' . $title . '", "caption": "Header Gadget"}';

                $userObj->update('blogs', ['Title' =>$title, 'Description' => $desc], ['blogID' => $blog->blogID]);
