...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['signup'])) {
        ...
        if(!empty($name) && !empty($email) && !empty($title) && !empty($blogUrl) && !empty($password)) {
            if(Validate::filterEmail($email)) {
            ...
            } elseif(strlen($title) > 150 OR strlen($title) < 1) {
                $error['title'] = "Title must be between 1 and 150 characters long.";
            } elseif (strlen($blogUrl) < 1 OR strlen($blogUrl) > 50) {
                $error['url'] = "This blog address is invalid or not supported.";
            } elseif (!preg_match('/^([a-z]+)$/', $blogUrl)) {
                $error['url'] = "This blog address is invalid or not supported.";
            } elseif($blogObj->blogExist($blogUrl)) {
                $error['email'] = "This blog is not available.";
            } elseif (strlen($password) < 4 OR strlen($password) > 16) {
                $error['password'] = "Password must be between 1 na 16 characters long.";
            } elseif (strlen($name) < 4 OR strlen($name) > 16) {
                $error['name'] = "Name must be between 1 na 16 characters long.";
            } else {
                if(isset($_FILES['profileImage'])) {
                    ...
                    $userID = $userObj->create('users', ['email' => $email, 'password' => $hash, 'fullName' => $name, 'profileImage' => $image]);
                    ...
                    if($userID) {
                        ...
                        $blogID = $userObj->create('blogs', [
                                ...
                                'CreatedBy' => $userID,
                                'Template' => $html,
                                'DefaultDemplate' => true]);

                        $userObj->create("blogsAuth", ['blogID' => $blogID, 'userID' => $userID, 'role' => 'Admin']);
